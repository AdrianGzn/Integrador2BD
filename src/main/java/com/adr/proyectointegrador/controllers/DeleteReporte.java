    ...
    void onMouseClickedEliminar(MouseEvent event) {
        if (idTextField.getText().trim().isEmpty()){
            ...
            try {
                ...
                if (App.getAplication().eliminarReporte(id).equals("No eliminado")){
                    Alert alert = new Alert(Alert.AlertType.INFORMATION);
                    alert.setTitle("Error");
                    alert.setContentText("No se ha podido eliminar el reporte.");
                    alert.showAndWait();
                }else {
                    Alert alert = new Alert(Alert.AlertType.INFORMATION);
                    alert.setTitle("Notificación");
                    alert.setContentText("Se ha eliminado el reporte correctamente.");
                    alert.showAndWait();
                    App.getStageView().close();
                }
